...
std::string GetStringJ(const unique_ptr<JoinFilterPushdownInfo>& pushdown_info) {
    ...
    std::string result;
    result += "JoinCond: " +
              GetStringL<idx_t>(pushdown_info->join_condition, [](const auto& idx) { return std::to_string(idx); }) +
              '\n';
    result += "MinMaxAggregates: " +
              GetStringL<unique_ptr<Expression>>(pushdown_info->min_max_aggregates,
                                                 [](const auto& el) { return GetStringE(el); }) +
              '\n';
...
std::string GetInfoLogicalOperator(LogicalOperator& op) {
    ...
    result +=
        "Children: " +
        GetStringL<unique_ptr<LogicalOperator>>(
            op.children,
            [](const auto& child) { return child->GetName() + "<" + LogicalOperatorToString(child->type) + ">"; }) +
        '\n';
    result += "Expressions: " +
              GetStringL<unique_ptr<Expression>>(op.expressions, [](const auto& expr) { return GetStringE(expr); }) +
              '\n';
    result += "Types: " + GetStringL<LogicalType>(op.types, [](const auto& type) { return type.ToString(); }) + '\n';

    result += "Estimated_cardinality: has=" + std::to_string(op.has_estimated_cardinality) +
              " val=" + std::to_string(op.estimated_cardinality) + '\n';
...
std::string GetInfoComparisonJoin(LogicalComparisonJoin& op) {
    ...
    result += "MarkTypes: " +
              GetStringL<duckdb::LogicalType>(op.mark_types, [](const auto& type) { return type.ToString(); }) + '\n';

    result += "DuplicateRliminatedCol: " +
              GetStringL<unique_ptr<Expression>>(op.duplicate_eliminated_columns,
                                                 [](const auto& expr) { return GetStringE(expr); }) +
              '\n';
...
std::string GetInfoLogicalGet(LogicalGet& op) {
    ...
    result += "Projections input: " +
              GetStringL<idx_t>(op.projected_input, [](const auto& el) { return std::to_string(el); }) + '\n';
...
std::string GetInfoLogicalAggregate(LogicalAggregate& op) {
    ...
    result +=
        "GroupStats: " +
        GetStringL<unique_ptr<BaseStatistics>>(op.group_stats, [](const auto& stat) { return stat->ToString(); }) +
        '\n';
    result +=
        "Validity: " + std::to_string(op.distinct_validity == TupleDataValidityType::CANNOT_HAVE_NULL_VALUES) + '\n';
